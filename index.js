...
const DEFAULT_DATA = {
  ...
  hours:[
    {id:1,day:"Saturday",time:"10:00 AM – 12:00 AM",closed:false},
    {id:2,day:"Sunday",time:"10:00 AM – 12:00 AM",closed:false},
    {id:3,day:"Monday",time:"10:00 AM – 12:00 AM",closed:false},
    {id:4,day:"Tuesday",time:"10:00 AM – 12:00 AM",closed:false},
    {id:5,day:"Wednesday",time:"10:00 AM – 12:00 AM",closed:false},
    {id:6,day:"Thursday",time:"10:00 AM – 1:00 AM",closed:false},
    {id:7,day:"Friday",time:"Closed",closed:true}
  ],
  contact:{
...
async function loadData() {
  ...
  try {
    ...
    DATA.menu = (menuRes.data?.length ? menuRes.data : DEFAULT_DATA.menu)
      .map(m => ({ ...m, desc: m.description ?? m.desc ?? '', imgKey: m.img_key ?? m.imgKey ?? '' }));
...
async function submitBooking(){
  ...
  if(!db){toast("Booking service is not available yet",true);return;}
...
async function updateBooking(id,status){
  ...
  if(!error){renderAdminBookings();toast("Booking marked as "+status);}
...
async function saveMenuItem(id){
  ...
  const{error}=await db.from('menu_items').update({name,cat,sub,description:desc,price,img_key:imgKey}).eq('id',id);
  if(!error){
    Object.assign(item,{name,cat,sub,desc,price,imgKey});
    renderMenu();flashRow("mrow_"+id);toast("Menu item saved ✓");
  } else toast("Save failed: "+error.message,true);
}
...
async function deleteMenuItem(id){
  ...
  renderAdminMenu();renderMenu();toast("Item deleted");
}
...
async function addMenuItem(){
  ...
  const{data,error}=await db.from('menu_items').insert({
    cat:'food',sub:'Mains',name:'New Item',description:'Item description',price:'30',img_key:'',sort_order:maxSort+1
  }).select().single();
  if(error||!data){toast("Failed to add item",true);return;}
  DATA.menu.push({...data,desc:data.description,imgKey:data.img_key});
  renderAdminMenu();renderMenu();toast("New item added — scroll down to edit");
}
...
async function saveBranch(id){
  ...
  const{error}=await db.from('branches').update({city,address,hours,status,img_key:imgKey}).eq('id',id);
  if(!error){
    Object.assign(b,{city,address,hours,status,imgKey});
    renderBranches();renderFindUs();flashRow("brrow_"+id);toast("Branch saved ✓");
  } else toast("Save failed: "+error.message,true);
}
...
async function deleteBranch(id){
  ...
  renderAdminBranches();renderBranches();renderFindUs();toast("Branch deleted");
}
...
async function addBranch(){
  ...
  const{data,error}=await db.from('branches').insert({
    city:'New City',address:'Address, City, Country',hours:'10:00 AM – 10:00 PM',status:'soon',img_key:'',sort_order:maxSort+1
  }).select().single();
  if(error||!data){toast("Failed to add branch",true);return;}
  DATA.branches.push({...data,imgKey:data.img_key});
  renderAdminBranches();renderBranches();renderFindUs();toast("Branch added");
}
  ...
  el.innerHTML=`<table class="at"><thead><tr><th>Day</th><th>Time / Label</th><th>Closed?</th><th></th></tr></thead>
    <tbody>${DATA.hours.map(h=>`
      <tr id="hrow_${h.id}">
        ...
        <td><button class="save-btn" onclick="saveHour(${h.id})">Save</button></td>
      </tr>`).join("")}
    </tbody></table>`;
}
...
async function saveHour(id){
  ...
  const{error}=await db.from('opening_hours').update({day,time,closed}).eq('id',id);
  if(!error){
    Object.assign(h,{day,time,closed});
    renderHours();flashRow("hrow_"+id);toast("Hours saved ✓");
  } else toast("Save failed: "+error.message,true);
}
...
async function saveContact(k){
  ...
  if(!error){renderFindUs();flashRow("crow_"+k);toast("Contact saved ✓");}
  else toast("Save failed: "+error.message,true);
}
...
async function saveHero(k){
  ...
  if(!error){renderHeroText();flashRow("hrow_"+k);toast("Hero text updated ✓");}
  else toast("Save failed: "+error.message,true);
}
...
async function uploadCustomImage(){
  ...
  if(!key){toast("Enter an image key",true);return;}
  if(!file){toast("Select an image file",true);return;}
  if(IMG[key]){toast("Key already exists — choose a different name",true);return;}
  ...
  if(dbErr){toast("Failed to save image record",true);return;}
